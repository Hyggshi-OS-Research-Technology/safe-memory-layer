...
export interface MemoryStoreOptions<K, V> {
  ...
  defaultTTL?: number;
  /** Interval in milliseconds between automatic cleanup runs (default: 10000, 0 to disable). */
  ...
  /** Enable statistics tracking (default: true). Set to false to reduce overhead. */
  statistics?: boolean;
  /** Enable event callbacks (default: true). Set to false to reduce overhead. */
  events?: boolean;
}
